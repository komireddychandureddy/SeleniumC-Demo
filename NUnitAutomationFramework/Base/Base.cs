...
*/
        ...
        public void SetTestResults()
        {
            ...
            if (status == TestStatus.Failed)
            {
                ...
                extent_test?.Value?.Fail("TestCase Status : Failed", CaptureScreenShot(driver.Value, Filename).Build());
        ...
        public static MediaEntityBuilder CaptureScreenShot(IWebDriver driver, string screenShotName)
        {
            ITakesScreenshot scr = (ITakesScreenshot)driver;
            string screenshot = scr.GetScreenshot().AsBase64EncodedString;

           // MediaEntityBuilder scr = MediaEntityBuilder.CreateScreenCaptureFromBase64String(screenshot, screenShotName).Build() ;
